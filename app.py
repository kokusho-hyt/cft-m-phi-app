...
def get_confined_concrete_props(fck, fsy, D, t, gamma_c, gamma_s, Ec):
    ...
    kc = max(0.85, 1.0 - 0.003 * fck) [cite: 40]
    return fcc, ecc, r, Ec, kc, fcd, fsyd

def sigma_concrete(eps, fcc, ecc, r, kc):
    if eps <= 0: return 0.0 
    x = eps / ecc [cite: 47, 401]
    return kc * (fcc * x * r) / (r - 1 + x**r) [cite: 375, 388]

def sigma_steel(eps, fsyd, Es):
    # ESのバイリニア特性 (硬化勾配 Es/100) を考慮 
    eps_syd = fsyd / Es [cite: 52]
    if abs(eps) <= eps_syd:
        return Es * eps
    else:
        sign = np.sign(eps)
        return sign * (fsyd + (abs(eps) - eps_syd) * (Es / 100.0)) [cite: 332, 415]
...
def generate_fibers_polar(D, t, n_r_conc=20, n_r_steel=5, n_theta=72):
    fibers = []
    d_theta = 2 * math.pi / n_theta [cite: 263]
    r_in = (D - 2*t) / 2.0
    r_out = D / 2.0
...
def analyze_section(phi, target_N, fibers, fsyd, fcc, ecc, r, Es):
    def calc_N_error(eps0):
        N_int = 0.0
        for f in fibers:
            eps_i = eps0 + phi * f['y'] [cite: 439]
            if f['mat'] == 'steel':
                N_int += sigma_steel(eps_i, fsyd, Es) * f['A']
            else:
                N_int += sigma_concrete(eps_i, fcc, ecc, r, 1.0) * f['A'] [cite: 440]
        return N_int - target_N

    try:
        # 収束範囲を広めに設定 (-0.5 to 0.5)
    ...
    for f in fibers:
        ...
        M_int += sigma * f['A'] * f['y'] * 1e-6 [cite: 444]
    return eps0_sol, M_int

def find_points_for_N(target_N_N, fibers, fsyd, fcc, ecc, r, D, t, Es):
    eps_syd = fsyd / Es
    y_45deg = - (D/2) * math.cos(math.radians(45)) [cite: 433]
    y_comp_edge = (D/2) - t
    eps_ultimate_limit = 2.0 * ecc # ES同期設定 [cite: 474, 456]
...
if st.sidebar.button(r"解析実行"):
    with st.spinner("解析中..."):
        ...
        st.markdown(f"**解析結果 (N = {target_N_kN} kN)**")
