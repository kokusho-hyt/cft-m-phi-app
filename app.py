...
# 2. ファイバー断面の生成 (極座標分割)
# ==========================================
def generate_fibers_polar(D, t, n_r_conc=25, n_r_steel=5, n_theta=72):
    fibers = []
...
def find_points_for_N(target_N_N, fibers, fsyd, fcc, ecc, r, D, t, Es):
    ...
    # 終局判定の修正: ピークひずみの 1.0倍 に変更
    eps_ultimate_limit = 1.0 * ecc 
    
    phi_max = 0.15 / D 
    phis = np.linspace(0, phi_max, 350)
...
if st.sidebar.button(r"解析実行"):
    with st.spinner("解析中..."):
        ...
        phis_raw, m_raw = [], []
        for p in np.linspace(0, 0.12/D, 200):
